...
- `print_check()` in `R/print.R` would mark an advisory with a yellow `!` bullet, as a first-class `hub_validations` member alongside the other check results. This already sits at a different level from the existing use of `!`: check-level validation warnings (the `$warnings` attached to an individual check) print as a sub-bullet indented beneath their check via `cli_alert_warning()`, so an advisory and a check-level warning never appear at the same level. Even so, to keep the classes clearly distinguishable we recommend changing the check-level validation warning symbol to match the validation-level warning symbol, `⚠` (`cli::symbol$warning`). That sub-bullet path is not currently deployed in any case: the one check-level warning that used it (coarser `compound_taskid_set`) was elevated into its check's main message.
